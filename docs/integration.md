...
* core's SQL database (latest only, updated every ledger)
  * in the case of captive-core (enabled via the `--in-memory` command line option) the ledger is maintained within the hcnet-core process and ledger-state need to be tracked as it changes via "meta" updates.
...
In captive-core mode, this entire per-ledger transition dataset is emitted by hcnet-core over a (optionally named) pipe, as an xdr encoded `LedgerCloseMeta` object.
